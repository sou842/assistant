...
export async function POST(req: Request) {
  try {
    ...
    const systemPrompt = [
      ...
      "3b. For schedule tasks: You HAVE FULL CAPABILITY to automate tasks and send messages asynchronously using the 'createScheduleTask' tool. NEVER say you cannot do this. CRUCIAL RULE FOR MODIFICATIONS: If the user asks to 'modify', 'edit', 'pause', 'resume', or 'update' an existing scheduled task or workflow, YOU MUST call 'listScheduleTasks' first to find the correct task ID, and then call 'updateScheduleTask' to modify it (e.g., to pause, set status to 'paused'; to resume, set status to 'active'). DO NOT call 'createScheduleTask' when modifying an existing task! Use 'createScheduleTask' ONLY when the user explicitly asks to create a brand new task. Use 'deleteScheduleTask' for deletion. Never claim create/update/delete succeeded unless the tool result returns success=true.",
      ...
      "9. For Vault (Data Storage): The Vault stores spreadsheets/tables (structured data), notes (unstructured data), and media galleries / albums. Use 'listVaultItems' to browse and 'getVaultItem' to read content. For creating/updating items, you MUST call 'getVaultNoteGuidelines' for notes OR 'getVaultSheetGuidelines' for spreadsheets beforehand to get the exact format. Do not use both at the same time. Always save spreadsheets, tables, lists, notes, or media galleries / albums in the Vault when asked. To create a media gallery/album, call 'createVaultItem' with type='gallery' or type='album' and content=array of media objects (each having: id, filename, url, mediaType, size).",
    ...
    try {
      ...
      const vaultTools = ['listVaultItems', 'getVaultItem', 'createVaultItem', 'updateVaultItem', 'deleteVaultItem', 'getVaultNoteGuidelines', 'getVaultSheetGuidelines'];
