...
- **Handling Questionnaires / Chatbots:**
  ...
  - **Read Questions in Subframes/Iframes:** The chatbot questions and inputs are often loaded inside an iframe or subframe. Look for frame headers (e.g., \`--- Frame [X] ---\`) in the page text to identify the active question (e.g., "How many years of experience do you have in Angular?").
  - **Complete All Inputs (including Contenteditable):** Inspect the modal/drawer (like \`.chatbot_Drawer\`) carefully. If the input field is a \`div\` with \`contenteditable="true"\` (usually inside class \`.textArea\` or having a placeholder like "Type message here..."), click it first to focus, then use the \`type\` action to enter the answer. **CRITICAL:** After using the \`type\` action to enter the answer, you MUST immediately find and click the "Save" or "Send" button to submit your answer, otherwise the application flow will be stuck.
  ...
  - **NEVER HALLUCINATE ANSWERS:** If a question asks for personal or professional info (e.g., current CTC, specific years of experience with a tool, current employer) that is NOT explicitly provided in the user's prompt, do NOT guess or make up an answer. Instead, IMMEDIATELY stop the task and ask the user for the missing information so they can provide it for you to remember in future applications.
